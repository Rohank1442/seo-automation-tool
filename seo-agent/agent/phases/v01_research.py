...
def run_research_phase():
    ...
    print("Example: 'A blog about indoor tomato gardening for apartment dwellers in New York, monetized with affiliate links.'")
    ...
    for round_num in range(1, 4):
        ...
        prompt = f"Extract parameters from the following project idea:\n\n{context_desc}"
    ...
    if confirm == 'edit':
        extraction.core_topic = input(f"Core Topic [{extraction.core_topic}]: ").strip() or extraction.core_topic
        extraction.target_audience = input(f"Target Audience [{extraction.target_audience}]: ").strip() or extraction.target_audience
        extraction.site_goal = input(f"Site Goal [{extraction.site_goal}]: ").strip() or extraction.site_goal
        extraction.geo_scope = input(f"Geo Scope [{extraction.geo_scope}]: ").strip() or extraction.geo_scope
        extraction.constraints = input(f"Constraints [{extraction.constraints}]: ").strip() or extraction.constraints
    ...
    print(f"Generated {len(clusters_resp.clusters)} clusters. Validating via Google Autocomplete...")
    ...
    while True:
        add_more = input("\nWould you like to add a custom topic cluster? (y/n) [n]: ").strip().lower()
        if add_more != 'y':
            break
    ...
    print("This will fetch keyword ideas, volume & difficulty metrics, and classify search intent.")
    print_separator()
    
    # Modifiers for autocomplete harvesting
    modifiers = [
        "", "for beginners", "how to", "best", "vs", "tips",
        "how", "what is", "why"
    ]
    ...
    for cluster in saved_clusters:
        ...
        for i in range(0, len(keywords_data), batch_size):
            ...
            intent_prompt = f"Classify the following list of keywords:\n{json.dumps(batch_kws)}"
            try:
                ...
                print(f"    - Error classifying batch starting at {i}: {e}. Keeping defaults.")
    ...
    for cluster in saved_clusters:
        ...
        kw_sample = [
            {"keyword": kw["keyword"], "intent": kw["intent"], "is_question": kw["is_question"]}
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        for c in saved_clusters:
            ...
            for comp in comps:
                f.write(f"  - [{comp['title'] or comp['url']}]({comp['url']}) (Word count: {comp['word_count']})\n")
            ...
            if gaps:
                for gap in gaps:
                    f.write(f"  - **{gap['topic']}** (Priority: {gap['priority'].upper()})\n")
                    f.write(f"    - *Reason*: {gap['reason']}\n")
                    f.write(f"    - *Recommended Format*: {gap['missing_format']}\n")
                    f.write(f"    - *Target Keywords*: {', '.join(gap['keyword_ideas'])}\n")
            else:
    ...
    for c_name, gaps in content_gaps.items():
        cluster_gap_counts[c_name] = sum(1 for g in gaps if g["priority"].lower() == "high")
        
    recommended_cluster = max(cluster_gap_counts, key=cluster_gap_counts.get)
    
    print(f"\nRecommended starting cluster: {recommended_cluster}")
    print(f"Reason: This cluster has the highest density of identified high-priority content gaps ({cluster_gap_counts[recommended_cluster]}).")
    print_separator("═")
